...
from settings import (
    ...
    MAP_UPDATE_INTERVAL,
    MAX_PATH_POINTS, PATH_MIN_DIST_MM,   # IMPROVEMENT 2 & 3
)
...
# Module-level variables track the last recorded position.  These live in the
# child process's own memory space, so there is no cross-process contention.
...
def _record_path_point(pss: ProcessSharedState, x_mm: float, y_mm: float) -> None:
    """Append (x_mm, y_mm) to the shared path ring buffer if the robot has
    moved at least PATH_MIN_DIST_MM since the last recorded point.

    Using a minimum-distance threshold keeps the buffer from filling up while
    the robot sits still or spins in place.
    """
    global _last_path_x, _last_path_y

    # Skip if the robot hasn't moved far enough since the last point.
...
def run_slam_process(pss: ProcessSharedState) -> None:
    ...
    IMPROVEMENT 2 & 3: step 4 also calls _record_path_point() so the UI can
    draw a breadcrumb trail showing where the robot has been.
    ...
    try:
        for raw_angles, raw_distances in lidar_driver.scan_rounds(lidar, scan_mode):
            ...
            # IMPROVEMENT 2 & 3: record position for path breadcrumb trail.
            _record_path_point(pss, x_mm, y_mm)

            # Copy the updated map into shared memory at a throttled rate.
            # Copying 1 MB on every scan would be expensive; MAP_UPDATE_INTERVAL
            # controls the maximum copy frequency (now 2x/s, up from 1x/s).
            now = time.monotonic()
